...
$.ajax({
  ...
  success: function(respObj) {
    if (respObj.success) {
      ...
      Z.check.localStorageSupport ? localStorage.setItem(user + "favoriteData", respObj.data) : console.log("Your browser is not support localStorage!");
...
$.ajax({
  ...
  dataType: 'json',
  data: {
    action: 'getpublicstrategy'
  },
  success: function(respObj) {
    if (respObj.success) {
      Z.check.localStorageSupport ? localStorage.setItem('publicStrategy', respObj.data) : console.log("Your browser is not support localStorage!");
      initPublicStrategy(JSON.parse(respObj.data), '#publicstrategy');
    }
  }
});
      ...
      if (index == 2) {
        $.ajax({
          ...
          success: function(respObj) {
            ...
            var tradedataPanel = new mdui.Panel('#tradedata .mdui-panel', {
              accordion: true
            });
          }
        });
          ...
          },
          {
            ...
            onClick: function() {
              $.ajax({
                ...
                data: {
                  "action": "deletetrade",
                  "data": JSON.stringify(tradearray)
                },
                success: function(respObj) {
                  if (respObj.success) {
                    mdui.snackbar({
...
$(document).on('confirm.mdui.dialog', '#editgroup',
  function() {
    ...
    var groupNameArrary = favoriteData.favorite.map((item, index, array) => {
      return item['name'];
    });
    if (newname == '') {
      mdui.snackbar({
    ...
    } else if (groupNameArrary.includes(newname)) {
      mdui.snackbar({
        message: '分组名已存在，请更换分组名称',
        timeout: 2000,
        position: 'top'
      });
    } else {
...
$(document).on('confirm.mdui.dialog', '#newgroup',
  function() {
    ...
    } else if (groupNameArrary.includes(newgroupname)) {
      mdui.snackbar({
        message: '分组名已存在，请更换分组名称',
        timeout: 2000,
        position: 'top'
...
$(document).on('click', '#publicstrategy button',
  function() {
    searchObj = $(this).data();
    searchObj.searchPage = searchPage;
    ...
    $.ajax({
      ...
      success: function(respObj) {
        ...
        if (data.stocks.length > 0) {
          ...
          var str = '';
          data.stocks.forEach(function(e) {
...
$(document).on('click', '#search-more-btn button',
  function() {
    ...
    $.ajax({
      ...
      success: function(respObj) {
        ...
        if (data.stocks.length > 0) {
          ...
          let domlist = document.querySelectorAll('#searchresult .chart');
          let list = [];
          for (var i = searchObj.searchPage * 12 - 12; i < domlist.length; i++) {
            list.push(domlist[i]);
          }
          list.forEach(function(e, i) {
            var chart = echarts.init(e);
